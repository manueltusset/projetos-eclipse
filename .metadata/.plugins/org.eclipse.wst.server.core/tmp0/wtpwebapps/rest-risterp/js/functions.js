...
function acessarSistema() {
	...
	request('GET', 'rest/usuarios/validar/' + user + '/' + senha, '', function(
			err) {
		alert(err);
	}, function(ok) {
...
function montarEmpresasCel() {
	...
	var json;
	request('GET',
			'rest/usuarios_empresas/empresas/' + document.cusuarioLogado, '',
			function(err) {
				alert(err);
			}, function(ok) {
				json = JSON.parse(ok);
...
function selEmpresa(linha) {
	...
	if (emp_sel) {
		...
		document.querySelector('#name-user-info').innerText = 'Seja Bem-Vindo(a) '
				+ document.usuarioLogado
				+ ' | Empresa: '
				+ document.empresaLogada
				+ ' | Cidade: '
				+ document.empresaCidade;
